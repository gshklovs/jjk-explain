...
- Continuity on work shots: no refs are sent, so keep the prop, the bench and the palette identical in the prompt text; a work shot may use `"image"` (a still of the hands or the prop) as its first frame, and `"objects"` if the exact shape matters (note: objects put the shot back on the reference model).
