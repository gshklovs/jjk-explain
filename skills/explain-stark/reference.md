...
Inventor: "a dark-haired man with a trimmed goatee in a dark tank top, no armor, at a steel workbench in a private workshop"
...
## Two kinds of shots: character shots and work shots (interleave them)
Every shot is one of two kinds, set per scene with `"shot": "character" | "work"` (seeded styles default to `character`; a script-level `"shot"` sets the default).
- CHARACTER shot: the face is on screen and the character speaks the line. Reference stills, the voice sample, the model's own lip-synced speech. This is the expensive path (reference-to-video, $0.05/s at 480P). Use it where the person matters: the reaction, the address to the viewer, the landing line.
- WORK shot: no face in frame. Hands on the part, the diagram being drawn in sand, the hologram he is turning, the workbench from above, the two students' hands on the disc, the ring of pins in close-up. Rendered on the cheap model (turbo, $0.025/s) with no character refs, and the line is dubbed over it as voice-over in the speaker's fish.audio voice (the narrator, the master, the scientist, the inventor, the AI). Nothing on screen can mouth the words, so a work shot is also where every bodiless voice (the AI, an off-screen narrator) belongs. A dubbed work shot may carry two speakers with tags in one line (`[stark] Give me the exploded view. [ai] Input shaft, eccentric bearing...`): Fish voices each part, so the ask and the answer can share the shot; a lip-synced character shot is one speaker only.
- The split is not wide versus close-up. It is face and interaction versus visualization and work. A close-up of hands is a work shot; a wide with his face reacting is a character shot.
- Interleave them: aim for about half the shots as work shots, never two character shots of the same person talking back to back without a work shot between, and put the explanation's mechanism beats on work shots (that is where the thing is shown) and the stakes and payoff on character shots.
- Continuity on work shots: no refs are sent, so keep the prop, the bench and the palette identical in the prompt text; a work shot may use `"image"` (a still of the hands or the prop) as its first frame, and `"objects"` if the exact shape matters (note: objects put the shot back on the reference model). On a turbo work shot with no refs, any mention of hands or a body invites the model to draw a whole stranger; if the shot needs no hands, write "nobody in the room, no person, no hands, no body anywhere in frame" outright, and if it needs hands, write "only two hands and forearms from the bottom edge, no head, no body".
- Cost: a six-shot lesson with three work shots costs roughly half of an all-character one.
Before rendering, show the user the shot list with each shot's kind and the estimated cost, and ask which shots should be character (lip-synced) and which work (dubbed); they may flip any. The default proposal should already interleave.

## Labels and reference visuals on work shots (the visual must teach)
A work shot is the explanation, so its picture has to be right and has to build with the line.
- Never ask the video model for text. Put `"labels"` on the scene instead: `[{"text": "FOUNDER", "cue": "founder"}, ...]`. The renderer draws each label in the style's HUD look and pops it in the instant its cue word is spoken (timed from the narration audio), stacked down the right edge; `"x"`/`"y"` fractions place one by hand. Use labels whenever the line enumerates (four channels, three hungers, the parts of a drive) so the items appear as they are named.
- Build in sync: write the prompt so what is named appears when it is named ("four small nozzles light one after another", "the lobes are drawn first, then the pins"); one element per beat of the sentence. Hands on a work shot do one specific thing per named part, in the label order, as one continuous movement ("the right hand places the intake, slides right, drops in the compressor discs, which start spinning..."); a single gesture ("a spread of hands") leaves them bobbing for the whole shot. Anything that moves in real life must be told to move ("the compressor rows spin fast about the shaft and keep spinning once placed"); the model animates only what the prompt names. A dubbed line must fit the clip: at most ~30 words per work shot, or the last frame freezes while the voice runs on.
- Name props literally, in words a stranger would draw correctly: "the red-and-gold armored boot with a thruster in its sole", not "the boot rig"; "a flat disc with a wavy lobed rim inside a ring of rollers", not "the drive". The first turbo work shot turned "boot rig" into a hologram of a hiking boot.
- Reference the visual, not the face: for a work shot that shows a specific thing (a hologram of the object, the diagram, the mechanism), fetch a clean reference image of that thing (web search, a product photo, a paper figure, a Wikipedia diagram) and pass it as the shot's `"image"` first frame with a prompt that restyles it ("rendered as a pale blue wireframe hologram lifting off the glass, the labels lit one by one"); turbo image-to-video keeps the cheap price and the shape stays true. When the reference is a colored or labelled diagram, say so in the prompt: "monochrome pale-blue wireframe; ignore the reference image's colors and labels", or the model paints the diagram's colors back in halfway through. `"objects"` refs do the same with more control but move the shot to the reference model.
- The line asks, the visual answers, in that order. When a character names a visual on a work shot ("Give me the exploded view", "Bring up the pins", "Draw the ring"), the words come first and the picture responds a beat later; a line that captions a picture already on screen ("Exploded view.") sounds like the screenwriter reading the prompt. So: write the line as a request or a command, start the prompt with the "before" state (an empty glass screen, the parts stacked as one assembly, a blank stretch of sand), and let the named thing appear or change after the first beat. If an `"image"` first frame is used, it must be the before state, not the result; a reference of the result goes in `"objects"` instead.

## The six-shot arc for character styles (half faces, half objects)
This is the default shape of a lesson in any character-led style (the tea shop, the garage, the workshop, the freeze-frame). Six shots after the title, at least three of them work shots with no lip sync:
1. CHARACTER, intro: the character brings up the topic in their own manner and exercises their quirk (the master pours tea and mis-hears the question, the scientist burps through the setup, the inventor sets a number and counts down, the narrator freezes the frame). Face on screen, lip-synced. The running example is named here.
2. WORK, zoomed visualization: the important object itself, large, with the character's hands or the AI's hologram or the sand diagram, and `"labels"` popping in as parts are named. The line asks for it first, the picture answers. No face.
3. WORK, the mechanism: the object doing the thing (the disc orbiting, the four nozzles firing in waves, the leaves sliding into rows), one variable changed on screen. No face.
4. CHARACTER, reaction or stakes: the character reacts, blames the equipment, asks the student, states the cost. Lip-synced.
5. WORK, the result: the outcome visualized on the object or the world (the arm lifting the block, the rig hovering, the cup on the shelf, the shop with a queue). No face; the line is the consequence.
6. CHARACTER, the concluding phrase or two: the takeaway said to the viewer or the student in character, then the still frame for the outro. Lip-synced.
Rules that come with it: source 2-3 real images of every important object before writing (web search, product photo, paper figure, encyclopedia diagram; save under `assets/ref/objects/<slug>/`) and use them on the work shots as `"objects"` (exact shape, reference model) or as a before-state `"image"` (turbo); name objects literally in prompts; character shots carry likeness and voice, work shots carry the explanation; never two character shots in a row without a work shot between. Cost: three character shots on the reference path plus three work shots on turbo is roughly half of six seeded shots. Show the user this list with each shot's kind before rendering and let them flip any.

## Hologram modes (do not run the same one twice in a row)
The hologram is the workshop's blackboard; it can be used several ways, and a lesson should mix them rather than repeat "he asks, the AI lists the parts" every time. Pick per shot, vary across a video and across videos:
1. THE AI READS IT: he asks ("Give me the exploded view"), the parts rise and light in turn, the AI names them, `"labels"` pop in on the cue words. Best for the first look at a thing with several parts.
2. HE BUILDS IT: hands only, no face; he assembles the concept in the hologram piece by piece. He gives the order ("Turbojet, front to back. Build it.") and the AI still calls the parts as they snap in ("Intake. Compressor, squeeze it..."), so the two voices stay in their roles: his hands and his commands, the AI's readouts. Both lines are dubbed in one shot (tagged `[stark]` then `[ai]`). Best for mechanisms with an order of assembly. Do not let him narrate a whole shot alone when the AI is in the room.
3. HE BREAKS IT: he pulls one part out of the hologram, or sets a variable wrong, and the model shows the failure (the disc jams, the arm slops, the network outputs garbage); the AI states the number that fixes it. Best for the cost, the condition, the one variable.
4. ON THE SUIT: the concept placed where it lives in the armor: the elbow module opens to show the drive, the boot thruster's turbine is drawn inside the boot, the helmet's targeting network hangs in the visor's HUD, the chest reactor's field lines. Use when the suit is where the viewer would meet the idea; the module is a prop, never his face. The armor stills (`suit-*.jpg`, `mk2-*.jpg`) are references for this shot only.
5. RUN IT: the hologram animates the simulation (the disc orbiting, air through the engine, pulses through the layers) and the AI reads one readout at the end ("eleven to one, sir"; "thrust exceeds weight by four percent").
Rules for all modes: the line asks or narrates the action, the picture answers a beat later; a monochrome pale-blue wireframe, "ignore the reference image's colors and labels" written into the prompt when a colored diagram is the object reference (the turbojet test turned pink and green halfway through); the referenced object's shape is exact; labels are drawn by the renderer, never asked of the model; hands and forearms only, no face.
