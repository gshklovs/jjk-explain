...
Default shape: the bible's "six-shot arc" (character intro with quirks, zoomed object visualization with labels, the mechanism, reaction, the visualized result, the concluding phrase), at least three work shots without lip sync, with 2-3 sourced images per important object. Visualizing the words: every work/snap shot shows the consequence the line claims (the verb, not the nouns), no text on anything in the picture (labels are drawn by the renderer), nobody talking in dubbed b-roll; an optional script-level `"prop"` paragraph keeps one object consistent when the story tracks one. See the bible's "Visualizing the words". No numbers or text in the footage: prompts never ask for signs, prices, counters, screens or handwriting (the model garbles them); show quantities physically and put words in `"labels"`; grep the prompts for digits and sign|price|menu|screen|text|number|written before rendering. Shot kinds: mark every scene `"shot": "character"` (face on screen, speaks, lip-synced on the reference path) or `"shot": "work"` (no face: hands, the part, the diagram, the hologram; rendered on turbo and dubbed with the speaker's fish.audio voice). Interleave them, roughly half work shots, mechanism beats on work shots, bodiless voices always on work shots. Then ASK THE USER (AskUserQuestion, one question, multi-select over the shots) which shots should be lip-synced character shots and which dubbed work shots, showing the proposed kind and the estimated cost per shot; apply their answer before rendering. See the bible's "Two kinds of shots". On work shots, use `"labels"` for anything enumerated (drawn by the renderer as each cue word is spoken) and a fetched reference image as `"image"` so the visual is the real thing restyled; see "Labels and reference visuals on work shots".
