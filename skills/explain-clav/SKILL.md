...
Default shape: the bible's six-shot arc (hook, the wrong way, the mechanism, the right way, the bar, the verdict), every shot `"shot": "snap"` (no face: b-roll, a side-by-side, a card, stock-style footage; turbo, dubbed by the Fish clone, cut to the line). `"shot": "character"` (face on screen, lip-synced on the reference path) is allowed only once a set of refs has proven out on a paid test shot (the likeness rule in the bible: never lip-sync someone who is not clearly him), and then for at most one or two shots. Then ASK THE USER (AskUserQuestion, one question, multi-select over the shots) which shots, if any, should be character shots, showing the proposed kind and the estimated cost per shot (snap ~$0.20-0.25, character ~$0.50 at 480P regular price); apply their answer before rendering.

Quirk budget, verified before rendering (the bible's "Quirk budget"): list each item beside the line that satisfies it. (1) one one-word verdict from his vocabulary as its own sentence (jester, chud, mogged, brutal, over, ascending, descending, cooked); (2) one rating on his scale with a reason, which may be the one number; (3) "bro" at least twice; (4) one self-deprecating aside; (5) one fake-serious pause ("Now look."); (6) one out-of-pocket line, an unprovoked verdict on someone in the story, usually right after the wrong way; (7) the concept's slogan rewritten in his words. If any item is missing, rewrite before rendering; a script that quotes the source's famous phrasing intact has failed item 7.
...
Model: snaps route to `h3-max-turbo` ($0.025/s) and are dubbed with Fish; a six-snap short is about $1.50 at 480P. Character shots use reference stills and the voice sample, so they route to `h3-max` reference-to-video ($0.05/s at 480P, about $0.50 each). Dry-run first and read the prompts.
Proving refs out: before the first character shot with a new set of stills, render one paid test shot (a one-scene script with `"shot": "character"` and those `"refs"`), look at a contact sheet, and keep character shots only if it is clearly him. The stills from the two source clips did not pass; the `clav-hd-*.jpg` defaults (1080p-native crops from a studio interview) came noticeably closer on one test shot (`out/zero-to-one-clav/hd-test/`), pending the user's verdict.
...
- Default refs: `clav-hd-1.jpg` to `clav-hd-5.jpg` (1080p-native crops from a studio interview, bust and medium, three-quarter profile) with `"seed": 1217`. The older `clav-face.jpg`, `clav-bust.jpg`, `clav-hands.jpg`, `clav-3q.jpg`, `clav-wide.jpg` (soft crops of the two source clips) failed the likeness test and are only for an explicit `"refs"` override. The renderer prepends "Image 1 to Image 5 show the same young man; keep his face, dark wavy hair, jawline and build consistent with them; his clothes and the room follow the description." Override with a top-level or per-scene `"refs"` / `"seed"`; `"refs": []` renders text-only. A default still missing from disk is skipped with a `[refs]` warning.
