...
- Default refs: `matan-face.jpg`, `matan-front.jpg`, `matan-3q.jpg` (1080p-native crops of two guest close-ups), `matan-set.jpg`, `matan-hands.jpg` (his studio: the bedsheet, the black table, the index card; scaled up from 960x540) with `"seed": 2007`. The renderer prepends "Image 1 to Image 5 show the same young man; keep his face, the mop of dark curly hair, round cheeks and build consistent with them; his clothes and the room follow the description." Override with a top-level or per-scene `"refs"` / `"seed"`; `"refs": []` renders text-only. A default still missing from disk is skipped with a `[refs]` warning.
- Extras: `matan-profile.jpg` (talking, profile), `matan-down.jpg` (looking down), `matan-brick.jpg` (three-quarter, brick wall).
...
Making more stills: the interview upload is age-gated (no formats without a login), so stills come from his own 1080p uploads and guest appearances; crop 16:9 windows at native resolution, crop out any picture-in-picture inset, and view every frame before keeping.
